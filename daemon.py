...
def doOligo():
    initializeRosetta()
    # open input file
    out=open('oligosoutputtemp.txt', 'w')
    try:
        inp=open("oligosinput", 'r')
    except:
        #raise file exception
        #try/except
    msa_inp=open("msainput.txt", 'w')

    while #not equal to 'MSA:':
        #write the msa to msa_inp
        #replace  dna input line with /n
        pass
        #replace POPEN
    subprocess( 'msa2profile', 'makefile')
    #get outout
    status,output=getStatusOutput(nw_prf2dna('profile'))
    #gets Degenerate-Protein muxed file
    if status:
        ##raise exception "prf2dna failed"
        ##system exit
    for x in output:
        ##inp.write(x)
        ##POPEN
        #report average waittime
        #
    subprocess('makefile','dnaworks', 'oligosinput.txt')
    #os.re
...
def daemonLoop():
    ...
    while (stillrunning):
        ...
        elif (os.path.isfile("oligosinput")):
            print "Daemon starting oligo generating job..."
            try:
                doOligo()
                print "Daemon completed oligo generating job"
            except Exception as e:
                print "The daemon crashed while performing the oligo job!"
                writeError(e.message)
            os.remove("oligosinput")
        else:
            # CUSTOM MODULE
